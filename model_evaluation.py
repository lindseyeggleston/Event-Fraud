...
def preprocess(filename):
    ...
    numeric_columns = ['body_length', 'name_length', 'num_order', 'num_payouts', 'sale_duration2', 'user_age', 'event_duration', 'creation_duration']
    ss = StandardScaler()
    X[numeric_columns] = ss.fit_transform(X[numeric_columns])
    X = X[numeric_columns] #shortcurt, we need to add categorical variables!!!
